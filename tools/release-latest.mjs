...
async function main() {
  // Clone repository
  await execSync(`git clone --single-branch --branch main ${gitUrl} ${deployPath}`);
  ...
  // Configure git
  await execSync('git config --global user.name "github-actions[bot]"');
  await execSync('git config --global user.email "github-actions[bot]@users.noreply.github.com"');
  ...
  await fs.writeFile(`<span class="math-inline">\{artifactPath\}</span>{version}.json`, jsonData);
  ...
  await execSync(`git commit -m "Release New Version: ${version}"`);
  await execSync(`git push "git@github.com:Amazefcc233/mimir-docs" main:main`);
}
